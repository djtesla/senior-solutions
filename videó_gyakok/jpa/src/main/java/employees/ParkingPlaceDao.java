...
public class ParkingPlaceDao {
    ...
    public	ParkingPlace findParkingPLaceNumber(int number) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        ParkingPlace parkingPlace =entityManager.createQuery("select p from ParkingPlace p where p.number =:number", ParkingPlace.class).setParameter("number", number).getSingleResult();
        entityManager.close();
        return  parkingPlace;
    }
